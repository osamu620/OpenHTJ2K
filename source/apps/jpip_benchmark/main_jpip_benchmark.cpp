...
ViewWindow make_view_window(const CodestreamIndex &idx, uint32_t gx, uint32_t gy, uint32_t radius,
                            float fsiz_ratio, bool whole_image) {
      ...
      for (uint8_t r = 0; r <= info.NL; ++r) {
        ...
        for (uint32_t p = 0; p < n; ++p) {
          if (keep.count(idx.I(static_cast<uint16_t>(t), c, r, p)))
            emit_precinct_databin(cs.data(), cs.size(), static_cast<uint16_t>(t), c, r, p, idx, locator,
                                  ctx, stream);
        }
      }
    }
...
// Reassemble the sparse codestream for `jpp` and time its decode.
//
// reuse_dec == nullptr : fresh decoder per call + invoke_line_based_stream.
//   Every call rebuilds the full tile tree (the codestream's precinct geometry
//   is set by the main header, so the sparse stream still carries all ~N
//   precincts even when only a few have bodies) — the timed invoke pays that
//   rebuild every frame.
// reuse_dec != nullptr : persistent decoder (enable_single_tile_reuse set by
//   the caller) + invoke_line_based_stream_reuse.  Every sparse stream shares
//   the original main header, so the tile tree built on the first (warm-up)
//   call is reused — the timed invoke no longer includes the rebuild.  This is
//   the path the WASM JPIP viewer (web/src/jpip_wrapper.cpp) already uses.
double decode_jpp(const std::vector<uint8_t> &jpp, const CodestreamIndex &idx, uint8_t reduce_NL,
                  open_htj2k::openhtj2k_decoder *reuse_dec) {
  DataBinSet set;
  ...
  if (reuse_dec) {
    ...
    try {
      dec.invoke_line_based_stream_reuse(cb, w, h, d, s);
    } catch (...) {
      return -1.0;
    }
...
int main(int argc, char **argv) {
  ...
  uint8_t reduce_NL  = 0;
  std::string csv_path;
  bool use_reuse = false;  // --reuse: persistent single-tile-reuse decoder
  ...
  // Persistent reuse decoder (when --reuse): all sparse streams of this image
  // share the original main header, so the tile tree built on the first decode
  // is reused on every later frame instead of being rebuilt from scratch.
  open_htj2k::openhtj2k_decoder reuse_decoder;
  if (use_reuse) reuse_decoder.enable_single_tile_reuse(true);
  open_htj2k::openhtj2k_decoder *rd = use_reuse ? &reuse_decoder : nullptr;
  ...
  if (!idx) {
    std::fprintf(stderr, "CodestreamIndex build failed\n");
    return EXIT_FAILURE;
  }
  ...
  if (!locator) {
    std::fprintf(stderr, "PacketLocator build failed\n");
    return EXIT_FAILURE;
  }
  ...
  std::printf("Image: %s (%u x %u, %llu precincts, reduce=%u)\n", infile.c_str(), cw, ch,
              static_cast<unsigned long long>(total_p), reduce_NL);
  std::printf("Foveation: fovea_r=%u  para_r=%u  para_ratio=%.3f  peri_ratio=%.3f\n", fovea_r, para_r,
              static_cast<double>(para_ratio), static_cast<double>(peri_ratio));
  ...
  // In --reuse mode, prime the tile-tree cache once (untimed) so every timed
  // decode below measures the warm steady state, not the one-off cold build.
  if (rd) decode_jpp(full_jpp, *idx, reduce_NL, rd);
  double full_decode_ms = decode_jpp(full_jpp, *idx, reduce_NL, rd);

  std::printf("Decode path: %s\n", use_reuse ? "single-tile-reuse (warm)" : "fresh decoder per frame");
  ...
  for (int yi = 0; yi < grid_n; ++yi) {
    for (int xi = 0; xi < grid_n; ++xi) {
      uint32_t gx = (grid_n == 1) ? cw / 2
                                  : static_cast<uint32_t>(static_cast<uint64_t>(xi) * (cw - 1)
                                                          / static_cast<uint64_t>(grid_n - 1));
      uint32_t gy = (grid_n == 1) ? ch / 2
  ...
  if (!csv_path.empty()) {
    ...
    if (f) {
      std::fprintf(f,
                   "gaze_x,gaze_y,fov_precincts,fov_bytes,fov_decode_ms,"
      ...
      for (const auto &r : results) {
        std::fprintf(f, "%u,%u,%zu,%zu,%.2f,%zu,%zu,%zu,%zu,%zu,%.2f,%.1f,%.1f\n", r.gx, r.gy,
                     r.fov_precincts, r.fov_bytes, r.fov_decode_ms, r.fovea_bytes, r.para_bytes,
                     r.peri_bytes, r.full_precincts, r.full_bytes, r.full_decode_ms,
